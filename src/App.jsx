    ...
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",fontFamily:"'Montserrat',sans-serif"}}>
      ...
      <div style={{background:"#0F172A",borderBottom:"2px solid #11CD87",padding:"0 28px",height:64,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:200}}>
        <div style={{display:"flex",alignItems:"center",gap:14}}>
          <img src="/logo-white.png" alt="LingvoConnect IELTS" style={{height:24,width:"auto",display:"block"}}/>
...
async function parseTestWithAI(text, type) {
  const OPENAI_KEY = import.meta.env.VITE_OPENAI_API_KEY || "";
  if (!OPENAI_KEY) throw new Error("No OpenAI API key configured.");

  const prompts = {
    Reading: `You are an IELTS test parser. Extract the full content from this IELTS Reading test PDF text and return ONLY valid JSON (no markdown, no explanation).

JSON format:
{
  "title": "string — test/passage title",
  "passages": [
    {
      "title": "string — passage title",
      "text": "string — full passage text, preserve paragraphs with \\n\\n",
      "questions": [
        {
          "id": 1,
          "type": "mcq|tfng|matching|short_answer|fill_blank|heading_match",
...
Rules:
- Detect question types: MCQ has A/B/C/D options; TRUE/FALSE/NOT GIVEN has tfng type; matching uses matching type; fill in blank uses fill_blank; heading matching uses heading_match; other short answer uses short_answer.
- Preserve the full passage text exactly.
- Include answer key if present in the PDF.
- If multiple passages exist, include all of them.

PDF TEXT:
${text.slice(0, 12000)}`,

    Writing: `You are an IELTS test parser. Extract writing prompts from this PDF and return ONLY valid JSON.

JSON format:
{
  "task1": {
    "title": "string",
    "prompt": "string — full task 1 prompt text"
  },
  "task2": {
    "title": "string",
    "prompt": "string — full task 2 essay question"
  }
}

If only one task is present, set the other to null.

PDF TEXT:
${text.slice(0, 8000)}`,

    Listening: `You are an IELTS test parser. Extract the listening test content from this PDF and return ONLY valid JSON.
      ...
      "questions": [
        {
          ...
          "text": "string — question or blank prompt",
          "options": [],
          "answer": ""
        }
      ]
...
function PDFImportModal({ onClose, onImport }) {
  ...
  const [stage,    setStage]    = useState("idle"); // idle | extracting | parsing | done | error
  const [errMsg,   setErrMsg]   = useState("");
  const [dragOver, setDragOver] = useState(false);

  const handleFile = f => {
    if (f && f.type === "application/pdf") { setFile(f); setErrMsg(""); }
    else setErrMsg("Please upload a PDF file.");
  };

  const run = async () => {
    if (!file) return;
    try {
      setStage("extracting"); setErrMsg("");
      const text = await extractTextFromPDF(file);
      if (!text || text.length < 50) throw new Error("Could not extract text from PDF. Make sure it's a text-based PDF, not a scanned image.");
      setStage("parsing");
      const parsed = await parseTestWithAI(text, type);
      setStage("done");
      onImport(type, parsed);
      onClose();
    } catch(e) {
      setStage("error");
  ...
  const stageLabel = { idle:"", extracting:"📄 Extracting text from PDF…", parsing:"🤖 AI is parsing test content…", done:"✅ Done!", error:"" };
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.55)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{...cardStyle({padding:36,width:"100%",maxWidth:520})}}>
        {/* Header */}
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}}>
          <div>
            <div style={{fontSize:20,fontWeight:800,color:C.s900}}>📄 Import from PDF</div>
            <div style={{fontSize:13,color:C.s400,marginTop:2}}>Upload an IELTS test PDF — AI will fill everything automatically</div>
          </div>
          <button onClick={onClose} style={{background:"none",border:"none",fontSize:22,cursor:"pointer",color:C.s400,lineHeight:1}}>×</button>
        </div>
          ...
          <div style={{display:"flex",gap:8}}>
            {["Reading","Writing","Listening"].map(t=>(
              <button key={t} onClick={()=>setType(t)} style={{
                flex:1, padding:"9px 0", border:`2px solid ${type===t?(t==="Reading"?C.teal:t==="Writing"?C.violet:C.amber):C.s200}`,
                borderRadius:10, background: type===t?(t==="Reading"?C.tealL:t==="Writing"?"#F3E8FF":C.amberL):"#fff",
                color: type===t?(t==="Reading"?C.teal:t==="Writing"?C.violet:C.amber):C.s600,
                fontWeight:700, fontSize:13, cursor:"pointer", transition:"all .15s",
              }}>{t==="Reading"?"📖":t==="Writing"?"✍️":"🎧"} {t}</button>
            ))}
          </div>
        </div>
        ...
        <div
          ...
          onClick={()=>!busy&&document.getElementById("pdf-file-input").click()}
          style={{
            border:`2px dashed ${dragOver?C.lc:file?C.brand:C.s200}`,
            borderRadius:14, padding:"32px 20px", textAlign:"center", cursor:busy?"default":"pointer",
            background: dragOver?C.lcL:file?C.brandL:"#FAFAFA", transition:"all .2s", marginBottom:16,
          }}>
          <input id="pdf-file-input" type="file" accept=".pdf" style={{display:"none"}} onChange={e=>handleFile(e.target.files[0])}/>
          {file ? (
            <>
              <div style={{fontSize:32,marginBottom:8}}>📄</div>
              <div style={{fontWeight:700,color:C.s900,fontSize:15}}>{file.name}</div>
              <div style={{fontSize:12,color:C.s400,marginTop:4}}>{(file.size/1024).toFixed(0)} KB · click to change</div>
            </>
          ) : (
            <>
              <div style={{fontSize:32,marginBottom:8}}>⬆️</div>
              <div style={{fontWeight:700,color:C.s600,fontSize:14}}>Drop PDF here or click to browse</div>
              <div style={{fontSize:12,color:C.s400,marginTop:4}}>Text-based PDFs only (not scanned images)</div>
            </>
          )}
        </div>
        ...
        {/* Progress */}
        {busy && (
          <div style={{background:C.brandL,borderRadius:10,padding:"12px 16px",marginBottom:16,fontSize:13,color:C.brand,fontWeight:600,display:"flex",alignItems:"center",gap:10}}>
        ...
        {/* Error */}
        {errMsg && (
          <div style={{background:C.roseL,borderRadius:10,padding:"10px 14px",marginBottom:16,fontSize:13,color:C.rose,fontWeight:600}}>
            ⚠️ {errMsg}
          </div>
        )}
        ...
        {/* Buttons */}
        <div style={{display:"flex",gap:10}}>
          <button onClick={onClose} style={{...btnStyle("secondary"),flex:1}} disabled={busy}>Cancel</button>
          <button onClick={run} disabled={!file||busy} style={{
            ...btnStyle("primary", !file||busy), flex:2,
            background: !file||busy ? C.s200 : `linear-gradient(135deg,${C.lc},#11CD87)`,
            boxShadow: !file||busy ? "none" : `0 4px 16px rgba(0,191,178,.35)`,
          }}>
            {busy ? stageLabel[stage] : "🚀 Parse & Auto-Fill"}
          </button>
        </div>
      </div>
    </div>
...
function AddTestManager() {
  ...
  const handlePDFImport = (type, parsed) => {
    setActiveType(type);
    if (type === "Reading") {
      setRTitle(parsed.title || "");
      if (parsed.passages?.length) {
        setRPassages(parsed.passages.map((p, i) => ({
      ...
      if (parsed.task1) { setWT1Title(parsed.task1.title || "Writing Task 1"); setWT1Prompt(parsed.task1.prompt || ""); }
      if (parsed.task2) { setWT2Title(parsed.task2.title || "Writing Task 2"); setWT2Prompt(parsed.task2.prompt || ""); }
    } else if (type === "Listening") {
      setLTitle(parsed.title || "");
      if (parsed.sections?.length) {
        setLSections(parsed.sections.map((s, i) => ({
          id: Date.now() + Math.random() + i,
  ...
  return (
    <div>
      {showPDFModal && <PDFImportModal onClose={()=>setShowPDFModal(false)} onImport={handlePDFImport}/>}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6,flexWrap:"wrap",gap:12}}>
        <div>
          <h2 style={{fontSize:22,fontWeight:800,color:C.s900,letterSpacing:"-0.03em",marginBottom:6}}>Section Builder</h2>
          <p style={{color:C.s400,fontSize:14}}>Build reading passages, listening sections, and writing prompts — then combine them into named Test Suites.</p>
        </div>
        <button onClick={()=>setShowPDFModal(true)} style={{
          ...
          boxShadow:`0 4px 16px rgba(0,191,178,.35)`,flexShrink:0,
        }}>
          📄 Import from PDF
        </button>
      </div>
